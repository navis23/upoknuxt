    ...
    <div class="md:ml-64 px-10 text-gray-700">
        ...
        <div class="flex gap-x-2 h-18 items-center rounded-lg bg-gray-800">
            ...
            <div class="w-full flex items-center">
                <FormKit
                type="search"
                placeholder="Cari berdasarkan nama transaksi disini . . . "
                v-model="searchField" 
            ...
            <table class="w-full">
                <thead>
                    <tr class="text-sm font-medium border-b border-gray-200">
                        <td class="py-5 font-semibold whitespace-nowrap">KODE TRANSAKSI</td>
                        <td class="py-5 px-4 font-semibold whitespace-nowrap">NAMA SUPPLIER</td>
                        <td class="py-5 px-4 font-semibold whitespace-nowrap">TRANSAKSI</td>
                        <td class="py-5 px-4 font-semibold text-right whitespace-nowrap">TOTAL TRANSAKSI</td>
                        <td class="py-5 px-4 font-semibold whitespace-nowrap">CATATAN</td>
                ...
                <tbody>
                    <tr v-for="(item, index) in transaksi" :key="index" class="border-b">
                        ...
                        <td class="p-4 text-sm whitespace-nowrap" :class="item.catatan == '' ? 'text-gray-400' : ''">
                            {{ item.catatan == '' ? 'tidak ada catatan' : item.catatan}}
                        </td>

                        <td class="p-4 whitespace-nowrap text-center">
                            <div class="flex gap-x-2 justify-center">
                                <button class="flex items-center gap-x-1 py-2 pl-3 pr-4 font-semibold text-sm rounded-md bg-teal-200 bg-opacity-30 text-teal-700 hover:bg-opacity-60">
...
<script setup lang="ts">
   ...
   const storeTransaction = useTransactionStore()
    ...
    const saveEditCategory = ( async () => {
        loadingOverlay.value = true
        await axios.post(`${urlHostApi}bengkel-api/api/transaksi/update`, {
            id : id.value,
            nama_kategori : nama_kategori.value,
        }, {
            headers: {
    ...
    const saveDeleteCategory = ( async () => {
        loadingOverlay.value = true
        await axios.post(`${urlHostApi}bengkel-api/api/transaksi/hapus`, {
            id : id.value
        }, {
        headers: {
                "Content-Type": "multipart/form-data",
